...
USER_FILE = "user.dat"
USER_WALLET = "wallet3.csv" #["Nome Conto","Entrate/Uscite","Causale","Data"]
		...
		while True:
			...
			if choice=="1":
				...
				if int(choice)<len(expiration_db)+1:
					...
					store(USER_WALLET,'a',[wallet_name,str(int(erased[1])*(-1)),erased[0]+" "+erased[2]+" "+text_old_date,time.strftime("%d/%m/%Y %H:%M.%S")])
					clear()
